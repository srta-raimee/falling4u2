...
def prepare_stratified_kfold(input_dir, output_dir, k_folds):
    ...
    SOURCE_IMG_DIR = os.path.join(input_dir, 'images')
    SOURCE_LABEL_DIR = os.path.join(input_dir, 'labels')
    ...
    print("Mapeando classes para estratificação (isso pode levar um momento)...")
    labels = []
    for img_file in tqdm(all_image_files, desc="Verificando labels"):
    ...
    for fold_idx, (train_indices, val_indices) in enumerate(kf.split(all_image_files, labels)):
        ...
        for idx in tqdm(train_indices, desc="Arquivos de Treino"):
        ...
        for idx in tqdm(val_indices, desc="Arquivos de Validação"):
            filename = all_image_files[idx]
...
if __name__ == '__main__':
    INPUT_DATA_DIR = 'le2i_things/LE2I_YOLO_FORMAT'
    NUM_FOLDS = 5
    OUTPUT_KFOLD_DIR = f'le2i_things/LE2I_kfold_stratified_{NUM_FOLDS}'
