...
def run_kfold_training(kfold_base_dir, output_csv, k_folds=5, epochs=200, batch_size=8, model_name="yolo11n.pt", patience=20):
    ...
    PATIENCE = patience
    EXPERIMENT_PREFIX = f'kfold_{os.path.basename(kfold_base_dir)}_{k_folds}folds_{epochs}ep'
    CSV_FOLDS_LOG_FILE = f'log_detalhado_{EXPERIMENT_PREFIX}.csv'
...
if __name__ == '__main__':
    LISTA_DE_FOLDS = [10]
    OUTPUT_CSV_FILE = 'resultados_kfold_stratified_200ep.csv'
    NUM_EPOCHS = 200
    BATCH_SIZE = 8
    MODELO = "yolo11n.pt"
    PATIENCE = 20
    ...
    for k in LISTA_DE_FOLDS:
        kfold_data_dir = f'kfold_dataset_stratified_{k}'
        
        print(f"\n============== INICIANDO EXPERIMENTO ESTRATIFICADO PARA K={k}, Lendo da pasta: {kfold_data_dir} ==============")
